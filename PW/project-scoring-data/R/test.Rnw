...
The criterion of comparision was Information Value. We performed equal frequency discretization to the number of bins which we earlier obtain from 2. and 3. methods. To comparision, we eventually picked up this equal frequency discretization number of bins which gives better IV result. 
\paragraph{}
The IV comparision in presented on the \textit{Figure 6.} below. 

\begin{figure}[h!]
\centering
<<fig.width=8, fig.height=4, out.width='.95\\linewidth'>>=
iv.comparision.df <- dget(paste0(wd, "/data/iv-comparision-df"))
ggplot(iv.comparision.df, aes(var.name, iv, group = cat.sgn, colour = cat.sgn)) + geom_line()
@
\caption{Comparision of Information Values of variables resulted from binning performed by means of 3 differend methods. }
\end{figure}
